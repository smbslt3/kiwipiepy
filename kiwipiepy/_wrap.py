...
```
    ...
    def __post_init__(self):
        if self.condition not in (None, 'any', 'vowel', 'applosive', 'continual'):
            raise ValueError(f"`condition` should be one of (None, 'any', 'vowel', 'applosive', 'continual'), but {self.condition}")
...
def _convert_oov_handling(oov_handling: Union[str, int]) -> Match:
    if oov_handling is None:
        return Match.OOV_CHR_MODEL
    elif oov_handling == 'rule':
        return Match.OOV_RULE_ONLY
    elif oov_handling == 'chr':
        return Match.OOV_CHR_MODEL
    elif oov_handling == 'chr_freq':
...
>>> kiwi = Kiwi()
# typos에 정의된 오타들을 교정 후보로 사용하여 형태소 분석 수행
>>> kiwi.tokenize('과재를 했다', typos=typos, typo_cost_threshold=2.) 
...
def _convert_typos(typos: Union[str, TypoTransformer]) -> TypoTransformer:
    import kiwipiepy
    if typos == 'basic': 
        return kiwipiepy.basic_typos
    elif typos == 'continual':
        return kiwipiepy.continual_typos
    elif typos == 'basic_with_continual':
        return kiwipiepy.basic_typos_with_continual
    elif typos == 'lengthening':
        return kiwipiepy.lengthening_typos
    elif typos == 'basic_with_continual_and_lengthening':
        return kiwipiepy.basic_typos_with_continual_and_lengthening
    elif typos == 'all':
        return kiwipiepy.basic_typos_with_continual_and_lengthening
    elif typos is None or isinstance(typos, TypoTransformer):
        return typos
    else:
        raise ValueError(f"`typos` should be one of ('basic', 'continual', 'basic_with_continual', 'lengthening', 'basic_with_continual_and_lengthening', 'all', TypoTransformer), but {typos}")
...
class ChrDataset(_ChrDataset):
    def __init__(self,
                 batch_size: int,
                 causal_context_size: int,
                 window_size: int,
                 dropout_prob: float = 0.0,
                 sample_without_weights: bool = False,
...
반대로 이 값을 낮추면 더 적은 후보를 탐색하여 속도가 빨라지지만 정확도는 낮아집니다. 초기값은 5입니다.
    ...
    oov_rule_scale: float = 4.0
    '''
    '''

    oov_rule_bias: float = 4.0
    '''
    '''

    oov_chr_bias: float = 0.0
    '''
    '''

    oov_global_weight: float = 75.0
    '''
    '''

    oov_local_weight: float = 3.0
    '''
    '''

    oov_global_min_freq: float = 4.0
    '''
    '''
...
enabled_dialects: Union[Dialect, str]
    ...
    def __init__(self, 
        ...
        if typos is not None or typo_cost_threshold != 2.5:
            warnings.warn("The `typos` and `typo_cost_threshold` arguments of `Kiwi.__init__` are deprecated since v0.23.0. "
                          "Please use the `typos` and `typo_cost_threshold` arguments of `Kiwi.analyze` and `Kiwi.tokenize` instead.", DeprecationWarning, 2)
...
```
        ...
        return super().analyze(text, top_n, match_options, False, blocklist, open_ending, allowed_dialects, dialect_cost, typos, typo_cost_threshold, pretokenized, override_config)
...
오타 교정시 고려할 최대 오타 비용입니다. 이 비용을 넘어서는 오타에 대해서는 탐색하지 않습니다. 기본값은 2.5입니다.
        '''
        warnings.warn("`Kiwi.typo_cost_threshold` is deprecated since 0.23.0.", DeprecationWarning, stacklevel=2)
        return
    ...
    def typo_cost_threshold(self, v:float):
        ...
        warnings.warn("`Kiwi.typo_cost_threshold` is deprecated since 0.23.0. Please use `typo_cost_threshold` argument of `Kiwi.analyze` and `Kiwi.tokenize` instead.", DeprecationWarning, stacklevel=2)
        return
...
oov_handling: str
    ...
    신조어나 사전에 등재되지 않은 단어를 처리하는 방법을 지정합니다. 'rule', 'chr', 'chr_freq' 중 하나의 값을 입력할 수 있습니다. 기본값은 'chr'입니다.

    * `'rule'`: 길이에 기반한 규칙으로 OOV에 점수를 부여합니다. 0.23.0 버전 이전까지 쓰이던 방식입니다.
    * `'chr'`: 경량 문자모델에 기반하여 OOV에 점수를 부여합니다. 이 방식은 규칙 기반 방식에 비해 OOV 탐지 성능이 크게 향상되었으며 0.23.0 버전부터는 기본값으로 사용됩니다. 
    * `'chr_freq'`: 경량 문자모델에 추가로 문자열 빈도 정보를 활용하여 OOV에 점수를 부여합니다. 입력 데이터 내에서 여러 번 등장하는 명사 패턴을 OOV로 탐지하는데 효과적입니다.

    자세한 내용은 <a href='#_8'>여기</a>를 참조하세요.

typos: Union[str, TypoTransformer]
    .. versionupdated:: 0.23.0
...
```
        ...
        riter = super().analyze(_zip_consequences(iter(text_chunks)), 1, Match.ALL, False, None, False, 0, 0., None, 0., None, self.global_config)
...
```
        ...
        if isinstance(text, str):
            ...
            return _space((super().analyze(text, 1, Match.ALL | Match.Z_CODA, False, None, False, 0, 0., None, 0., None, self.global_config), text))
        else:
            if reset_whitespace: text = map(_reset, text)
            return map(_space, super().analyze(text, 1, Match.ALL | Match.Z_CODA, True, None, False, 0, 0., None, 0., None, self.global_config))
